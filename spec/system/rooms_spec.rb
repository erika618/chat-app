...
# 「チャットルームを削除すると、関連するメッセージがすべて削除されていること」をテスト
...
RSpec.describe "チャットルームの削除機能", type: :system do
  ...
  it 'チャットルームを削除すると、関連するメッセージがすべて削除されていること' do
    ...
    # create_listを用いることで、messageテーブルのレコードをまとめて生成できる。
    ...
    # find_linkメソッドで「"チャットを終了する",ref:room_path(@room_user.room)」を取得して、クリック。
